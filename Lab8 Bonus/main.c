...
TaskHandle_t cont_handle;
TaskHandle_t periodic_handle;
void delay(void)
{
...
{
	portBASE_TYPE xHigherPriorityTaskWoken = pdFALSE;
	UBaseType_t cont = uxTaskPriorityGet(cont_handle);
	UBaseType_t periodic = uxTaskPriorityGet(periodic_handle);
	vTaskPrioritySet(cont_handle,periodic);
	vTaskPrioritySet(periodic_handle,cont);
	xSemaphoreGiveFromISR(semaphore, &xHigherPriorityTaskWoken);
	GPIOIntClear(GPIOA_BASE, GPIO_INT_PIN_0);
	portEND_SWITCHING_ISR(xHigherPriorityTaskWoken);
}
...
int main()
{
	...
	xTaskCreate(CounterTask, "init", 240, NULL, 2, cont_handle);
	xTaskCreate(LedTogglerTask, "init", 240, NULL, 3, periodic_handle);
